...
func getNodeGroupHealthIssueReason(code string, message string) string {
	...
	} else if strings.Contains(message, "Cannot exceed quota for InstanceProfilesPerAccount") {
		return expinfrav1.EKSNodegroupInstanceProfilesPerAccountExceededReason
	}
...
func isRecoverableFailureReason(failureReason string) bool {
	return failureReason == expinfrav1.EKSNodegroupInsufficientCapacityReason ||
		...
		failureReason == expinfrav1.EKSNodegroupInstanceProfilesPerAccountExceededReason
}
